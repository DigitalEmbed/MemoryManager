...
int main(){
  /*!
    Allocating...
  */
  uint8_t* ui8SampleA = (uint8_t*) DataBank_malloc(sizeof(uint8_t), 20);
  uint8_t* ui8SampleB = (uint8_t*) DataBank_malloc(sizeof(uint8_t), 40);
  ...
  */
  DataBank_free(&ui8SampleA, sizeof(uint8_t), 20);
  ...
  */
  if (DataBank_checkFragmentation(sizeof(uint8_t)) == DATA_BANK_STATUS_FRAGMENTED_MEMORY){                /*!< If memory pool is fragmented... */
    printf("ATENTION: The memory pool is fragmented!\n");                                                 /*!< Print an error message and the maximum size of allocation... */
    printf("Possible maximum size allocation: %d\n", DataBank_getMaxFreeSpace(sizeof(uint8_t)));          /*!< You can treat the problem any way you want! */
  }

  /*!
    Trying allocate memory...
  */
  ui8SampleC = (uint8_t*) DataBank_malloc(sizeof(uint8_t), 40);
